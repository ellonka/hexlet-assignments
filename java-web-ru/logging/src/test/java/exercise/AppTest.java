...
class AppTest {
    ...
    void testSignUp() throws IOException {
        CloseableHttpClient client = HttpClients.createDefault();
        HttpPost postRequest1 = new HttpPost(baseUrl + "/users/list");
        List<NameValuePair> params = new ArrayList<>();
        params.add(new BasicNameValuePair("firstName", "Fokus"));
        params.add(new BasicNameValuePair("lastName", "Pokus"));
        params.add(new BasicNameValuePair("email", "fokus.pokus@gmail.com"));
        postRequest1.setEntity(new UrlEncodedFormEntity(params));
        CloseableHttpResponse response1 = client.execute(postRequest1);
        assertThat(response1.getCode()).isEqualTo(200);
    }
