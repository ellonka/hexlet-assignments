...
public final class UserController {
    ...
    public static Handler createUser = ctx -> {
        ...
        Validator<String> emailValidator = ctx.formParamAsClass("email", String.class)
                .check(email -> EmailValidator.getInstance().isValid(email), "Email введен неверно");
        Validator<String> passwordValidator = ctx.formParamAsClass("password", String.class)
                ...
                .check(pass -> pass.length() > 4, "Пароль должен содержать не менее 4 символов");
        ...
        user.save();
        ctx.sessionAttribute("flash", "Новый пользователь успешно создан");
        ctx.redirect("/users");
        // END
    };
}
